...
namespace vpp
{
  template <unsigned WS, typename F, typename GD>
  std::pair<vfloat2, float> lk_match_point_square_win(i_float2 p, vfloat2 tr_prediction,
                                                      F A, F B, GD Ag,
                                                      float min_ev)
  {
    int ws = WS;
    ...
    vfloat2 prediction_ = p + tr_prediction_;
    ...
    for (int k = 0; k < 30 && nk.norm() >= 0.1; k++)
    {
      ...
      for(int r = -hws; r <= hws; r++)
      {
        for(int c = -hws; c <= hws; c++)
        {
          vfloat2 n2 = v + i_int2(r, c) * factor;
          {
            auto g = gs[i];
            float dt = (float(as[i].x) - B.linear_interpolate(n2));
