#ifndef VPP_MIEL_DETECTOR_HPP_
# define VPP_MIEL_DETECTOR_HPP_
...
namespace vpp
{
  ...
  namespace MIEL_internals
  {

    template <typename N>
    inline
    int miel_score(const N& nbh)
    {
      typedef typename N::value_type V;
      V v = 2 * nbh(0, 0);
      int saillance = INT_MAX;

      auto f = [&] (r1, c1, r2, c2) {
        saillance = std::min(saillance, std::abs(v - n(r1, c1) - n(r2, c2)))
      };
    ...
    template <typename N>
    bool is_miel_keypoint(const N& n, int th)
    {
      typedef typename N::value_type V;

      return miel_score(n) > th;
    }
...
#    include <arm_neon.h>
    ...
    struct miel_simd // ARM NEON version
    {
...
#    else
    ...
    struct miel_simd // Fallback version
    {
...
#  endif
    ...
    template <typename V>
    void miel_detector9_simd(image2d<V>& A, std::vector<vint2>& keypoints, int th,
...
#pragma omp for
      for (int r = 0; r < nr; r++)
      {
        ...
        {
          v hi,lo;
          v a0,a1,a2,a3,a4,a5,a6,a7,a8;

          v possible;
          if (m_row) // If we have a mask.
          {
...
#pragma omp critical
    ...
    void miel_detector9(image2d<V>& A, image2d<U>& B, int th)
    {
      int nc = A.ncols();
      int nr = A.nrows();
      int pitch = A.pitch();
...
#pragma omp critical
  ...
  template <typename V>
  void miel_detector9_scores(image2d<V>& A,
                             int th, 
                             ...
                             std::vector<int>& scores)
  {
    ...
    for (int i = 0; i < keypoints.size(); i++)
      scores[i] = MIEL_internals::miel_score(box_nbh2d<V, 7, 7>(A, keypoints[i]), th);
  }

  template <typename V>
  int miel_detector9_score(image2d<V>& A,
                           int th, 
                           vint2 p)
  {
    return MIEL_internals::miel_score(box_nbh2d<V, 7, 7>(A, p), th);
  }

  template <typename V>
  std::vector<vint2> miel_detector9(image2d<V>& A,
                                    int th,
                                    const image2d<unsigned char>& mask,
                                    std::vector<int>* scores)
  {
    std::vector<vint2> kps;
    MIEL_internals::miel_detector9_simd(A, kps, th, mask);
    if (scores)
      miel_detector9_scores(A, th, kps, *scores);
    return std::move(kps);
  }

  template <typename V, typename F>
  auto miel_detector9_maxima(image2d<V>& A,
                                           int th,
                                           const image2d<unsigned char>& mask,
                                           std::vector<int>* scores,
                                           F maxima_filter)
  {
    ...
    {
      auto p = kps[i];
      int s = MIEL_internals::miel_score(box_nbh2d<V, 7, 7>(A, p), th);
      scores_img(p) = s;
    }
...
#pragma omp parallel for
  ...
  template <typename V, typename F>
  auto miel_detector9_maxima2(image2d<V>& A,
                              int th,
                              const image2d<unsigned char>& mask,
                              std::vector<int>* scores,
                              F maxima_filter)
...
#pragma omp parallel for
  ...
  template <typename V>
  std::vector<vint2> miel_detector9_blockwise_maxima(image2d<V>& A,
                                                     int th,
                                                     int block_size,
                                                     const image2d<unsigned char>& mask,
                                                     std::vector<int>* scores)
  {
    return miel_detector9_maxima(A, th, mask, scores,
  ...
  template <typename V>
  std::vector<vint3> miel_detector9_blockwise_rank(image2d<V>& A,
                                                   int th,
                                                   int block_size,
                                                   ...
                                                   std::vector<int>* scores)
  {
    return miel_detector9_maxima2(A, th, mask, scores,
                                 [=] (auto& S, auto& kps)
                                 {
                                   int nc = S.ncols();
                                   int nr = S.nrows();
  ...
  template <typename V>
  std::vector<vint2> miel_detector9_local_maxima(image2d<V>& A,
                                                 int th,
                                                 const image2d<unsigned char>& mask,
                                                 std::vector<int>* scores)
  {
    return miel_detector9_maxima(A, th, mask,
                                 scores,
                                 [] (auto& img, auto& kps)
                                 {
                                   std::vector<vint2> lms;
