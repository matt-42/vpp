...
/**
 * @brief The foreach_videoframe1 struct
 */
...
/**
 * @brief The foreach_videoframe2 struct
 */
...
/**
 * @brief The foreach_frame struct
 */
...
/**
 * @brief hough_feature_matching_paint
 * @param T_theta
 * @param link
 * @param scale
 * @param type_video
...
void fast_dht_matching(int mode, Theta_max discr, Sclare_rho scale,
                   ...
                   Type_Lines type_line, Frequence freq, With_Kalman_Filter wkf,
                       With_Transparency wt,With_Entries we,
                   OPTS... options)
{
    ...
    int T_theta = get_theta_max(discr);
    int wkff = get_WKF(wkf);

    if(mode==dense_ht)
    {
        hough_image(T_theta,acc_threshold,link);
    }
